...
<div class="top_box">
    ...
    <div class="top_nav">
		...
		您好！<?php echo $_username;?> [<?php echo $_admin == 1 ? ($CFG['founderid'] == $_userid ? '网站创始人' : '超级管理员') : ($_aid ? '<span class="f_blue">'.$AREA[$_aid]['areaname'].'站</span>管理员' : '普通管理员'); ?>]  &nbsp;&nbsp;&nbsp;&nbsp; <a href="./" target="_blank">网站首页</a>
		</div>
		<?php if($_admin == 2) {
...
?> <a href="javascript:void(0);" onClick="self.top.location.href='?file=logout'">安全注销</a> <?php } else { ?>
		...
		<a href="?file=left&menu=3" target='left'>扩展管理</a>
		 ...
		 <a href="?file=kygl" target='main'>客源管理</a>
		 <?php } ?>
    </div>
    </div>
</div>
