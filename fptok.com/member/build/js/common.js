...
function Dnotification(id, url, icon, title, content) {
	 if(window.webgluiNotifications) {
		 if(webgluiNotifications.checkPermission()==1) {
			 window.onclick = function() {
				window.webgluiNotifications.requestPermission(function() {
					if(webgluiNotifications.checkPermission()==0) {						
						var N = window.webgluiNotifications.createNotification(icon, title, content);
						N.replaceId = id;N.onclick = function() {window.focus();window.top.location = url;N.cancel();};N.show();
					}
				});
			 };
		 } else if(webgluiNotifications.checkPermission()==0) {	
			var N = window.webgluiNotifications.createNotification(icon, title, content);
			N.replaceId = id;N.onclick = function() {window.focus();window.top.location = url;N.cancel();};N.show();
		 }
	 }
