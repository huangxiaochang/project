...
switch($action) {
	case 'add':
		if($MG['sell_limit'] && $limit_used >= $MG['sell_limit']) dalert(lang($L['info_limit'], array($MG['sell_limit'], $limit_used)), $_userid ? $MODULE[2]['linkurl'].$AJ['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$AJ['file_my']);
		if($MG['day_limit']) {
			...
			if($do->pass($post)) {
				...
				foreach(explode('|', $swf_upload) as $v) {
					if($v) {
						...
						$db->query("UPDATE {$AJ_PRE}newhosue_6  SET thumb='$v' WHERE itemid=$do->houseid");
	...
	$usernames = get_broker($_userid) ? get_broker($_userid).',"'.$_username.'"' : '"'.$_username.'"';
	$condition.= " AND username IN ($usernames)";
         ...
         * @author 房产
         */
       $lists[$k]['is_appo'] = house_id_exists($v['itemid']) ? true : false;
		}
	break;
}
function house_id_exists($id) {
        global $db;
		return $db->get_one("SELECT appo_list_id FROM {$db->pre}appolist WHERE house_id = $id and appo_site='sale'");
	}
if($_userid) {
	$nums = array();
	for($i = 1; $i < 5; $i++) {
