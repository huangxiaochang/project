...
class spread {
	...
	function _add($post) {
		...
		$this->db->query("INSERT INTO {$this->table_price} (word,sell_price,buy_price,company_price,editor,edittime) VALUES('$post[word]','$post[newhouse_price]','$post[sell_price]','$post[rent_price]','$post[buy_price]','$post[company_price]','$_username','$AJ_TIME')");
	}
	...
	function _edit($post) {
		foreach($post as $k=>$v) {
			...
			$this->db->query("UPDATE {$this->table_price} SET word='$v[word]',newhouse_price='$v[newhouse_price]',sell_price='$v[sell_price]',rent_price='$v[rent_price]',buy_price='$v[buy_price]',company_price='$v[company_price]' WHERE itemid='$k'");
		}
	}
