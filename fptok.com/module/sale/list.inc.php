...
$param = $_GET['param'];
	...
	if(!empty($keyword))
	{
		...
		$lst1.= "-k".htmlentities(urlencode($keyword));
		$condition.=" and (`title` like '%$keyword%' or `address` like '%$keyword%' or `housename` like '%$keyword%' or `telephone` like '%$keyword%')";
	}
	if(!empty($k))
	{
		//$keyword = iconv('utf-8','gbk' , $k);
		$lst1.= "-k".htmlentities(urlencode($k));
		$condition.=" and (`title` like '%$k%'  or `address` like '%$k%' or `housename` like '%$k%' or `telephone` like '%$k%')";
	}
