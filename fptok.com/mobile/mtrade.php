...
if($action == 'update') {
	...
	switch($step) {
		case 'edit_price'://修改价格||确认订单||修改为货到付款
			...
			if($AJ['trade'] && $_trade == '') mobmsg(lang($L['trade_msg_pay_bind'], array($AJ['trade_nm'])), '?action=bind');
			if($submit) {
				$fee = dround($fee);
	...
	$AJ['trade'] or mobmsg($L['trade_msg_secured_close']);
	...
	if($submit) {
		...
		if($trade) {
			if($AJ['trade'] == 'alipay' && !is_email($trade) && !is_mobile($trade)) mobmsg(lang($L['trade_bind_error'], array($AJ['trade_nm'])));
		...
		$head_name = lang($L['trade_bind_title'], array($AJ['trade_nm']));
	}
