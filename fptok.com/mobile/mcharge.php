...
function get_reason_url($r = array()) {
	...
	switch($arr[0]) {
		...
		break;
		case 'trade':
			if(is_numeric($arr[1])) $url = 'mtrade.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['amount'], AJ_KEY.'CG', 600);
		break;
		case 'trades':
			$url = 'mtrade.php?action=muti&step=pay&auth='.encrypt($arr[1], AJ_KEY.'CG', 600);
		break;
		case 'group':
...
switch($action) {
	...
	case 'repay':
		...
		$bank = $r['bank'];
		$orderid = $itemid;
		$receive_url = AJ_PATH.'mobile/mcharge.php';//$MOD['linkurl'].'charge.php';
	...
	case 'confirm':
		...
		if($auto) $goto = 1;
		if(isset($goto)) {
			$receive_url = AJ_PATH.'mobile/mcharge.php';
