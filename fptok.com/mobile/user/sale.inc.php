		 ...
		 $post['thumb2'] = $thumbs[2];
			...
			if($AJ['mail_fabu']){
	               $linkurl = $MOD['linkurl'].'show.php?itemid='.$do->itemid;
					$subject = '你好，我发布了一条二手房'.$post['title'];
				...
				if($fee_add) {
				 ...
				 if($post['autophoto'] && $MODULE[12]['name'] && $AJ['gl_photoauto'] && !$AJ['ftp_remote']){
		        ...
		        $db->query("INSERT INTO {$AJ_PRE}photo_12 (catid,title,introduce,keyword,thumb,username,addtime,editor,edittime,ip,status) VALUES ('$pcatid','$ptitle','$ptitle','$keyword','$pthumb','$_username','$AJ_TIME','$_username','$AJ_TIME','$AJ_IP','$pstatus')");
				$itemid = $db->insert_id();
                $db->query("REPLACE INTO {$AJ_PRE}photo_data_12 (itemid,content) VALUES ('$itemid', '$ptitle')");
                $item = $db->get_one("SELECT * FROM {$AJ_PRE}photo_12 WHERE itemid=$itemid");
                $item['itemid'] = $itemid;
                $linkurl = itemurl($item);
                
				$thumb = mnew_filecopy($thumb);
				$db->query("UPDATE {$AJ_PRE}photo_12 SET linkurl='$linkurl' WHERE itemid=$itemid");
				$db->query("INSERT INTO {$AJ_PRE}photo_item_12 (item,introduce,thumb) VALUES ('$itemid','$title','$thumb')");
				$photos = 1;
				if($thumb1){
					$thumb1 = mnew_filecopy($thumb1);
					 $db->query("INSERT INTO {$AJ_PRE}photo_item_12 (item,introduce,thumb) VALUES ('$itemid','$title','$thumb1')");
					 $photos = 2;
				}
				if($thumb2){
					$thumb2 = mnew_filecopy($thumb2);
					$db->query("INSERT INTO {$AJ_PRE}photo_item_12 (item,introduce,thumb) VALUES ('$itemid','$title','$thumb2')");
					$photos = 3;
				}
				$db->query("UPDATE {$AJ_PRE}photo_12 SET items='$photos' WHERE itemid=$itemid");
