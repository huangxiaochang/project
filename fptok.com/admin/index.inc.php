...
switch($action) {
	case 'cache':
		...
		if($step == 1) {
			...
			cache_clear('allcity');
			cache_city();
			msg('分站缓存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 7) {
			cache_clear('fields');
			cache_fields();
			cache_clear('option');
	...
	case 'main':
		if($submit) {
           ...
           $aj = file_get(AJ_CACHE.'/aj.txt');
           if(!$aj) file_put(AJ_CACHE.'/aj.txt', $MODULE[1][linkurl].timetodate($AJ_TIME, 6));
		...
		$notice_url = decryptp('UGgMI155ACddNwEpVSYBLAF4DSgLLwo/AGZQPgc9DjpZags9U3MCewAzVDwBMAVwDnVQcgBuCj8Jclc2VCkGI1BoDCc', 'aijiacms').'?version='.AJ_VERSION.'&release='.AJ_RELEASE.'&lang='.AJ_LANG.'&charset='.AJ_CHARSET.'&install='.$install.'&os='.PHP_OS.'&soft='.urlencode($_SERVER['SERVER_SOFTWARE']).'&php='.urlencode(phpversion()).'&mysql='.urlencode(mysql_get_server_info()).'&url='.urlencode($AJ_URL).'&site='.urlencode($AJ['sitename']).'&auth='.strtoupper(md5($AJ_URL.$install.$_SERVER['SERVER_SOFTWARE']));
