...
$https_host = get_env('host');
if($city) {
	list($cityid, $city_domain) = explode('|', $city);
	$cityid = intval($cityid);
	if(strpos(AJ_PATH, $https_host) === false && strpos($city_domain, $https_host) === false) {
		$cf = $db->get_one("SELECT * FROM {$AJ_PRE}city WHERE domain='https://".$https_host."/'");
		if($cf) {
	...
	if(strpos(AJ_PATH, $https_host) === false) {
		$cf = $db->get_one("SELECT * FROM {$AJ_PRE}city WHERE domain='https://".$https_host."/'");
		if($cf) {
			set_cookie('city', $cf['areaid'].'|'.$cf['domain'], $AJ_TIME + 30*86400);
			$cityid = $cf['areaid'];
		...
		if($AJ['city_ip'] && !defined('AJ_ADMIN') && !$AJ_BOT) {
			...
			while($rs = $db->fetch_array($result)) {
				if(preg_match("/".$rs['name'].($rs['iparea'] ? '|'.$rs['iparea'] : '')."/i", $iparea)) {
					...
					$cityid = $rs['areaid'];
					if($rs['domain']) dheader($rs['domain']);
					$cf = $r;
					break;
				}
			}
