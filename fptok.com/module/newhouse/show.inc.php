...
//历史价格图
$video=$db->pre.'video_14';
$vd = $db->get_one("SELECT video FROM $video WHERE houseid=$itemid");
...
$r = $db->get_one("SELECT COUNT(*) AS num FROM $a a  LEFT JOIN $b b  ON b.item = a.itemid   where a.houseid=$itemid {$condition} and status=3 ");  	
...
if(!$pagesize || $pagesize > 100) $pagesize = 30;
	...
	$pages = housedpages($r['num'], $page,$at, $pagesize);
$tjw=$db->query("SELECT  a.itemid, houseid,linkurl,addtime,title,b.item,b.itemid,b.thumb,b.introduce ,b.mianji FROM $a a  LEFT JOIN $b b  ON b.item = a.itemid   where a.houseid=$itemid  {$condition} and status=3 ORDER BY b.itemid desc LIMIT $offset,$pagesize");
