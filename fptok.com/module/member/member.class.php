...
class member {
	...
	function is_member($member) {
		...
		if($MOD[email_register])if(!$this->is_email(trim($member['email']))) return false;
		if($MOD[email_register])if($this->email_exists(trim($member['email']))) return $this->_($L['member_email_reg']);
