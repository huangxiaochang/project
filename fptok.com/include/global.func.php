...
$db->query("INSERT INTO {$db->pre}sms (mobile,message,word,editor,sendtime,code,encode) VALUES ('$mobile','$message','$word','$_username','$AJ_TIME','$code','$encode')");
  ...
  function NewSms($uid,$pwd){
	   global $db, $AJ;
    $url="http://service.winic.org/webservice/public/remoney.asp?";
    $data = "uid=%s&pwd=%s";
    $uid = urlencode($AJ['sms_uid']);
	$pwd = urlencode($AJ['sms_key']);
    $rdata = sprintf($data, $uid, $pwd);
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_POST,1);
    curl_setopt($ch, CURLOPT_POSTFIELDS,$rdata);
    curl_setopt($ch, CURLOPT_URL,$url);
...
function cat_pos2($CAT, $str = ' &raquo; ', $target = '') {
	global $MODULE, $db;
	if(!$CAT) return '';
	$arrparentids = $CAT['arrparentid'].','.$CAT['catid'];
	$arrparentid = explode(',', $arrparentids);
...
function get_mainarea($areaid) {
	...
	$areaid = intval($areaid);
	$are = array();
	$result = $db->query("SELECT areaid,areaname,map FROM {$db->pre}area WHERE parentid=$areaid ORDER BY listorder,areaid ASC", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$are[] = $r;
	}
...
function pages($total, $page = 1, $perpage = 20, $demo = '', $step = 3) {
	...
	if($demo) {
		...
		if(defined('AJ_REWRITE') && RE_WRITE && $_SERVER["SCRIPT_NAME"] && strpos($AJ_URL, '?') === false) {
			...
			if(defined('AJ_ADMIN') && strpos($demo_url, 'sum=') === false) $demo_url = str_replace('page=', 'moduleid='.$moduleid.'sum='.$items.'&page=', $demo_url);
			// var_dump($demo_url);
		}
	}
	$pages = '';
	include AJ_ROOT.'/api/pages.'.((!$AJ['pages_mode'] && $page < 100) ? 'default' : 'sample').'.php';
// var_dump('pages.'.((!$AJ['pages_mode'] && $page < 100) ? 'default' : 'sample').'.php');
	return $pages;
}
...
//表单值
function getbox_indexval($name,$boxtype,$value,$tb){
global $db;
$options = array();
$v = $db->get_one("SELECT * FROM {$db->pre}fields WHERE name='$name' and tb='$tb' ");
$file=$v['option_value'];
...
$string .= ' <a href="'.'/house/'.'list-l'.$_v.'.html"'.$target.'>'.$option[$_v].'</a>';
}
break;
case 'select':
$string = $option[$value];
...
function get_agent($userid) {
	...
	$r = $db->get_one("SELECT * FROM  {$db->pre}company  WHERE userid='$companyid'");
      $username=$r['username'];
	return $username;
}
//获取会员companyid
...
function get_broker($userid) {
	global $MODULE, $db;
	$result = $db->query("SELECT username FROM {$db->pre}member where companyid='$userid'");
