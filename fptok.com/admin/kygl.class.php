...
class kygl {
	...
	function is_member($username) {
		return $this->db->get_one("SELECT userid FROM {$this->pre}kygl WHERE username='$username'");
	}

	function count_admin() {
		$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->pre}kygl WHERE groupid=1 AND admin=1 ");
		return $r['num'];
	}
	...
	function set_admin($username, $admin, $role, $aid) {
		...
		if($admin == 1) $aid = 0;
		$this->db->query("UPDATE {$this->pre}kygl SET groupid=1,admin=$admin,role='$role',aid=$aid WHERE userid=$userid");
	...
	function move_admin($username) {
		...
		if($r && $r['admin'] > 0) {			
			...
			$admin = $r['admin'] == 1 ? 2 : 1;
			$this->db->query("UPDATE {$this->pre}kygl SET admin=$admin WHERE username='$username'");
			return true;
		} else {
	...
	function delete_admin($username) {
		...
		if($r) {
			...
			$this->db->query("UPDATE {$this->pre}kygl SET groupid=$groupid,admin=0,role='',aid=0 WHERE userid=$userid");
	...
	function get_one($id) {
        return $this->db->get_one("SELECT * FROM {$this->pre}kygl WHERE `userid`=$id");
	}
	...
	function get_list() {
		...
		if($page > 1 && $sum) {
			...
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->pre}kygl ");
			$items = $r['num'];
		}
		$pages = pages($items, $page, $pagesize);
		$admins = array();
		$result = $this->db->query("SELECT * FROM {$this->pre}kygl ORDER BY userid ASC LIMIT $offset,$pagesize");
